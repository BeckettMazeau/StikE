...
void setup(void) {
  ...
  Serial.flush();
  tft.init();
  Serial.println("tft.init() succeeded");
  tft.setRotation(1); // your 90° rotation
  tft.writecommand(0x2A); // CASET
  tft.writedata(0x00);
  tft.writedata(0x1A); // column start = 26
  tft.writedata(0x00);
  tft.writedata(0x69); // column end = 105 (26+76-1... wait, 26+76=102)  tft.fillScreen(TFT_BLACK);
...
void loop() {
  if (targetTime < millis()) {
    targetTime = millis() + 1000;
    Serial.println(">>> Entering rainbow_fill");
    rainbow_fill();
    Serial.println(">>> rainbow_fill returned");
